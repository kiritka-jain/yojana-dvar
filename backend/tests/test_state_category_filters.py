...
def test_central_plus_state_specific_inclusion(matcher):
    ...
    profile_mh = ProfileInput(age=30, gender="Female", state="Maharashtra", income=100000, is_bpl=True, life_stage="general")
    ...
    assert any(s.scheme_id == "mukhyamantri-majhi-ladki-bahin-yojana" for s in mh_specific)

def test_state_isolation_between_states(matcher):
    """Verify that state-specific schemes do not leak into another state."""
    profile_up = ProfileInput(age=12, gender="Female", state="Uttar Pradesh", income=100000, life_stage="student")
    res_up = matcher.match_profile(profile_up)
    
    up_specific = [s for s in res_up.schemes if "kanya-sumangala" in s.scheme_id]
    assert len(up_specific) >= 1
    
    profile_tn = ProfileInput(age=12, gender="Female", state="Tamil Nadu", income=100000, life_stage="student")
    res_tn = matcher.match_profile(profile_tn)
    up_in_tn = [s for s in res_tn.schemes if "kanya-sumangala" in s.scheme_id]
    assert len(up_in_tn) == 0

def test_union_territory_matching(matcher):
    """Verify UT matching (e.g. Delhi free bus scheme)."""
    profile_delhi = ProfileInput(age=25, gender="Female", state="Delhi", income=150000, life_stage="general")
    res_delhi = matcher.match_profile(profile_delhi)
    
    delhi_schemes = [s for s in res_delhi.schemes if s.scheme_id == "delhi-free-dtc-bus-travel-scheme"]
    assert len(delhi_schemes) == 1
    
    profile_punjab = ProfileInput(age=25, gender="Female", state="Punjab", income=150000, life_stage="general")
    res_punjab = matcher.match_profile(profile_punjab)
    delhi_in_punjab = [s for s in res_punjab.schemes if s.scheme_id == "delhi-free-dtc-bus-travel-scheme"]
