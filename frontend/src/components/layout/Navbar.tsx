    ...
    <header className="sticky top-0 z-50 glass-nav transition-all duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-18 py-2">
          ...
          >
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-saffron-100 text-saffron-800 border border-saffron-200">
                  <Sparkles className="w-3 h-3 mr-1 text-saffron-600" />
                  AI Gateway
                </span>
              </div>
          ...
          <nav className="hidden md:flex items-center gap-1 lg:gap-2">
            <Link
              ...
              className={`px-3.5 py-2 rounded-lg text-sm font-medium transition-colors ${
                isActive('/') 
                  ? 'bg-saffron-100/80 text-saffron-800 font-semibold' 
                  : 'text-charcoal-700 hover:text-saffron-700 hover:bg-cream-200'
              }`}
            >
              {t('navHome')}
            </Link>
          ...
          {/* Right Action: Language Switcher + User Info */}
          <div className="flex items-center gap-3">
            {/* Language Switcher Pill */}
            <button
              onClick={toggleLanguage}
              aria-label="Toggle language between Hindi and English"
              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-saffron-300 bg-cream-100 hover:bg-saffron-50 text-xs font-semibold text-charcoal-800 shadow-sm transition-all duration-200 hover:scale-105 active:scale-95"
            >
              <Globe className="w-3.5 h-3.5 text-saffron-600" />
              <span className={language === 'en' ? 'font-bold text-saffron-700' : 'text-charcoal-400'}>
                EN
              </span>
              <span className="text-charcoal-300">|</span>
              <span className={language === 'hi' ? 'font-bold text-forest-700 text-sm' : 'text-charcoal-400'}>
                हिंदी
              </span>
            </button>
